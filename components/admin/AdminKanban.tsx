...
const KanbanColumn: React.FC<KanbanColumnProps> = ({ title, status, items, onUpdateStatus, onAddTask }) => (
    ...
    <div className="h-[calc(100vh-140px)] flex flex-col">
       ...
       {loading ? (
         ...
         <div className="flex gap-6 overflow-x-auto pb-4 h-full">
            <KanbanColumn title="To Do" status="todo" items={tasks.filter(t => t.status === 'todo')} onUpdateStatus={updateStatus} onAddTask={openAddTask} />
            <KanbanColumn title="In Progress" status="in-progress" items={tasks.filter(t => t.status === 'in-progress')} onUpdateStatus={updateStatus} onAddTask={openAddTask} />
            <KanbanColumn title="Review" status="review" items={tasks.filter(t => t.status === 'review')} onUpdateStatus={updateStatus} onAddTask={openAddTask} />
            <KanbanColumn title="Done" status="done" items={tasks.filter(t => t.status === 'done')} onUpdateStatus={updateStatus} onAddTask={openAddTask} />
         </div>
       )}
