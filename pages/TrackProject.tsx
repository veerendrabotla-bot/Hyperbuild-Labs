  ...
  const handleTrack = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .eq('is_active', true) // Critical: Block access if globally inactive
        .order('created_at', { ascending: false })
        .limit(1)
        .single();

      if (error) {
        showError('Access Denied. Ensure project is active and credentials match.');
      } else if (data) {
      ...
      <div className="max-w-4xl mx-auto px-4">
        ...
        {!project ? (
          <Card className="max-w-md mx-auto shadow-2xl border-brand-100 p-8">
            <form onSubmit={handleTrack} className="space-y-6">
              <div className="text-center mb-6">
                ...
                <h4 className="font-black text-slate-900 text-lg">Secure Verification</h4>
              ...
              <Button type="submit" className="w-full py-4 text-lg font-black shadow-xl shadow-brand-500/20" isLoading={loading} leftIcon={<Search size={18}/>}>Unlock Project Ledger</Button>
            </form>
          </Card>
        ) : (
          <div className="animate-fadeIn space-y-8">
            ...
            {project.show_financials && (
              <Card className="p-10 bg-slate-900 text-white border-none shadow-2xl relative overflow-hidden">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mb-10 relative z-10">
                    <div>
                       <p className="text-[10px] text-slate-400 font-black uppercase mb-2 tracking-widest">Project Value</p>
                       <p className="text-3xl font-black text-white">{getCurrencySymbol(project.currency)}{project.total_amount.toLocaleString()}</p>
                    </div>
                    <div>
                       <p className="text-[10px] text-brand-400 font-black uppercase mb-2 tracking-widest">Amount Settled</p>
                       <p className="text-3xl font-black text-brand-400">{getCurrencySymbol(project.currency)}{project.paid_amount.toLocaleString()}</p>
                    </div>
                    <div>
                       <p className="text-[10px] text-red-400 font-black uppercase mb-2 tracking-widest">Remaining Balance</p>
                       <p className="text-3xl font-black text-red-400">{getCurrencySymbol(project.currency)}{(project.total_amount - project.paid_amount).toLocaleString()}</p>
                    </div>
                 </div>
                 
                 <div className="space-y-3 relative z-10 bg-white/5 p-6 rounded-2xl border border-white/10">
                    <div className="flex justify-between text-[11px] font-black uppercase tracking-widest">
                       <span className="text-slate-400">Payment Fulfillment Progress</span>
            ...
            {project.show_lifecycle && (
              <Card className="p-10 border-slate-200">
                <div className="flex items-center justify-between mb-12">
                  <h3 className="font-black text-slate-900 uppercase tracking-widest text-xs">Milestone Execution Timeline</h3>
            ...
            {/* Digital Assets matrix */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
               {project.show_repo && (
                 <Card className="flex flex-col h-full hover:border-brand-400 transition-all p-10 border-slate-200">
                    <div className="bg-slate-900 w-16 h-16 rounded-2xl flex items-center justify-center text-brand-400 mb-8 shadow-2xl">
                      <Code2 size={32} />
                    </div>
                    <h4 className="text-2xl font-black text-slate-900 mb-3 tracking-tight">Engineering Hub</h4>
                    <p className="text-sm text-slate-500 mb-10 leading-relaxed font-medium">Direct secure endpoint to the technical environment and repository.</p>
                    {project.repo_link ? (
                      <a href={project.repo_link} target="_blank" rel="noopener noreferrer" className="mt-auto">
                        <Button variant="outline" className="w-full py-5 font-black text-sm uppercase tracking-widest border-2" rightIcon={<ExternalLink size={18}/>}>Access Repository</Button>
                      </a>
                    ) : (
                      <div className="mt-auto p-5 bg-slate-50 rounded-2xl border border-dashed border-slate-200 text-center text-[11px] font-black uppercase tracking-widest text-slate-400">Environment Provisioning in Progress</div>
                    )}
                 </Card>
               )}
               ...
               {project.show_live && (
                 <Card className="flex flex-col h-full hover:border-brand-400 transition-all p-10 border-slate-200">
                    ...
                    <h4 className="text-2xl font-black text-slate-900 mb-3 tracking-tight">Live Staging Preview</h4>
                    <p className="text-sm text-slate-500 mb-10 leading-relaxed font-medium">Observe the real-time production build and interface developments.</p>
                    {project.live_link ? (
                      <a href={project.live_link} target="_blank" rel="noopener noreferrer" className="mt-auto">
                        <Button variant="primary" className="w-full py-5 font-black text-sm uppercase tracking-widest shadow-xl shadow-brand-500/20" rightIcon={<ArrowRight size={18}/>}>Launch Staging Feed</Button>
                      </a>
                    ) : (
                      <div className="mt-auto p-5 bg-slate-50 rounded-2xl border border-dashed border-slate-200 text-center text-[11px] font-black uppercase tracking-widest text-slate-400">Deployment Pending Final Review</div>
                    )}
                 </Card>
               )}
            </div>
            ...
            <div className="text-center pt-8">
               <button onClick={() => setProject(null)} className="group text-slate-400 hover:text-brand-600 text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center mx-auto border border-slate-200 px-6 py-3 rounded-full hover:bg-white hover:shadow-md">
